...
async function submit() {
    ...
    try {
        ...
        if (e.response?.data?.errors) {
            ...
            serverError.value = e.response?.data?.message ?? 'Registration failed. Please try again.'
        }
    }
}
</script>
...
<template>
    <div>
        ...
        <form @submit.prevent="submit" class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Store / workspace name</label>
                <input
            ...
            <button
                ...
                class="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors mt-2"
            >
