...
const refImagePreview = ref<string | null>(null)

function onRefImageSelected(e: Event) {
    const file = (e.target as HTMLInputElement).files?.[0]
    if (!file) return
    refImageFile.value = file
    const reader = new FileReader()
    reader.onload = ev => { refImagePreview.value = ev.target?.result as string }
...
async function regenerateImage() {
    ...
    try {
        if (refImageFile.value) {
            await seoStore.regenerateFromReference(activePost.value.id, refImageFile.value, refPrompt.value || undefined)
            toast.success('New image generated from your reference')
        } else {
                        ...
                        <div class="mb-6">
                            ...
                            <div v-if="showImageTools" class="mt-3 bg-purple-50 border border-purple-200 rounded-xl p-4">
                                ...
                                <div class="mt-3 flex items-center gap-3">
                                    <input ref="refImageInput" type="file" class="hidden"
                                        accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" @change="onRefImageSelected" />
                                    <button @click="refImageInput?.click()" type="button"
                                        class="text-xs px-3 py-1.5 border border-purple-300 rounded-lg text-purple-700 hover:bg-purple-100 transition-colors">
                                        🖼 {{ refImageFile ? 'Change reference' : 'Add reference image' }}
                                    </button>
                                    <div v-if="refImagePreview" class="flex items-center gap-1.5">
                                        <img :src="refImagePreview" alt="" class="w-8 h-8 rounded object-cover border border-purple-200" />
                                        <button @click="clearRefImage" class="text-xs text-purple-500 hover:text-purple-700">✕ remove</button>
                                    </div>
                                </div>
                                <p class="text-[11px] text-purple-500 mt-1">
                                    With a reference image, AI looks at it and recreates a new image in that style guided by your description.
                                </p>
                                ...
                                <div class="flex items-center gap-2 mt-3">
                                    <button @click="regenerateImage" :disabled="regenerating"
                                        ...
                                        {{ regenerating ? '⏳ Generating…' : (refImageFile ? '✨ Generate from reference' : '✨ Generate with NVIDIA FLUX') }}
                                    </button>
